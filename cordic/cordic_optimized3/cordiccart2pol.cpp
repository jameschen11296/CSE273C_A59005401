...
#pragma HLS INTERFACE ap_vld port=x
	...
	cordiccart2pol_label0:for(int i=0;i<rot_num;i++){
		if(yfix>0){
			temp=xfix;
			xfix=xfix+(yfix*Kvalues[i]);
			yfix=yfix-(temp*Kvalues[i]);
			thetares=thetares+angles[i];
		}
		else if(yfix<0){
			temp=xfix;
			xfix=xfix-(yfix*Kvalues[i]);
			yfix=(temp*Kvalues[i])+yfix;
			thetares=thetares-angles[i];
		}
	}
