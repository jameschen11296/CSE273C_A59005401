...
constexpr int m_size = 144 * 8;  // Must be a multiple of 8.
  ...
  try {
    ...
    q.submit([&](auto &h) {
      ...
      h.template parallel_for<c_calc>(range(M, P), [=](auto index) {
        ...
        int col = index[1];
        auto input_ptr1 = a.get_pointer();
        auto input_ptr2 = b.get_pointer();
        auto output_ptr = c.get_pointer();

        float sum = 0.0f;
        // Compute the result of one element of c
        for (int i = 0; i < width_a/8; i++) {
            sum += PrefetchingLSU::load(input_ptr1 + (i*8) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+1) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+1) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+2) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+2) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+3) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+3) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+4) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+4) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+5) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+5) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+6) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+6) * P );
            sum += PrefetchingLSU::load(input_ptr1 + (i*8+7) + row * N)*PrefetchingLSU::load(input_ptr2 + col + (i*8+7) * P );
        }

        output_ptr[row*P+col] = sum;
      });
    });
